...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{driveWaveform}
    \caption{Torquer driving waveform}
    \label{fig:driveWV}
\end{figure}

\Cref{fig:driveWV} shows the driving waveform for the torquers. As the current spikes the torquer flip can be seen in the jump in the magnetic field. After the initial spike, the current decays as the capacitor discharges until the driver is switched off. The magnetic field intensity decays as the current decays but does not drop back to its previous value.
