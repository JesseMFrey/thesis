...
The Current waveforms also show changes due to torquer saturation. For the first flip the current initially increases at a slower rate. This because, on the Alnico1 B-H curve, the slope is steaper closer to the B-axis resulting in a higher inductance. As the torquer reaches saturation the inductance of the torquer goes down and the rate of change of current increases. For the subsaquent torquer flips the torquer is already saturated so the inductance is high and the driving current reaches its peak much faster.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{torquer-waveform-setup}
