...
The \ac{ACDS} operates as part of the \ac{ARC} and must work with other systems to perform its mission. The \ac{ACDS} must interact with other systems in three key ways, sensor data collection, operational status reporting, and mission control. All of these interactions must be tested to make sure that they will function properly in flight.

The \ac{ACDS} system does not directly connect to any sensors. To get sensor data in flight the \ac{ACDS} communicates with the \ac{LEDL} board. 


\subsubsection{Detumble Flip Test}
